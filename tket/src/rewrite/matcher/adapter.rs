...
pub struct HugrMatchAdapter<'m, M: CircuitMatcher> {
...
impl<'m, M: CircuitMatcher> HugrMatchAdapter<'m, M> {
...
impl<H: HugrView> Circuit<H> {
    fn get_op_args(&self, node: H::Node, checker: &LineConvexChecker<H>) -> Option<Vec<OpArg>> {
        ...
        for io in inputs.zip_longest(outputs) {
            ...
            match io {
                ...
                Left((inp, inp_type)) => {
                    ...
                    let const_f64 = get_const_f64(node, inp, self.hugr())?;
...
fn enqueue_extended_matches<M: CircuitMatcher, H: HugrView<Node = Node>>(
    circ: &Circuit<H>,
    ...
    if !new_subcircuit.try_extend(new_node, checker) {
        // non-convex match
    ...
    let Some(args) = circ.get_op_args(new_node, checker) else {
        // Unsupported op
        queue.push_back(current_match);
        return all_matches;
    };
...
fn get_const_f64<H: HugrView>(node: H::Node, port: IncomingPort, host: &H) -> Option<f64> {
...
mod tests {
    ...
    fn test_hugr_match_adapter_const_f64() {
        ...
        let match_ = matcher
            ...
            .exactly_one()
            .unwrap();
        assert_eq!(match_.0.node_count(), 1);
    }
}
